...
export default function EventTalkCard(props) {
  ...
  useEffect(() => {
    if (imageLink === undefined) {
      axios.get(`club/competition/${props.element.id}/`).then((res) => {
        const storage = getStorage();
        if (JSON.parse(res.data.image_links)[0]) {
          getDownloadURL(
            ref(storage, JSON.parse(res.data.image_links)[0])
          ).then((link) => setImageLink(link));
        } else {
  ...
  var finale =
    datee.getMonth() + 1 + '/' + datee.getDate() + '/' + datee.getFullYear();
        ...
        <Card className="h-100" style={{ borderRadius: '10px' }}>
          ...
          >
            <Card.Img
              ...
              src={
                imageLink ? imageLink : 'https://via.placeholder.com/350x200'
              }
              variant="top"
            />
          </div>
          ...
          <Card.Body className={styles.cardinner + ' px-2'}>
            ...
            <Card.Text className={styles.text}>
              <p>
                {isMobile
                  ? truncate(strippedText, 100)
                  : truncate(strippedText, 290)}
              </p>
            </Card.Text>
            ...
            <Card.Subtitle className={styles.bottom + ' d-flex text-muted '}>
              <SvgIcon height="15px" src="calendar_datee.svg" width="15px" />

              <p className={styles.text2}>. Fill before {finale}</p>
            </Card.Subtitle>
          </Card.Body>
        </Card>
      </Link>
